<script>
	...
	export let isTeacher = false; // ✅ 선생님이면 왼쪽
	export let name = '';
	export let badges = [];
	...
	const current_date = formatAMPM(new Date());

	// ✅ 선생님(왼쪽) / 학생(오른쪽)
	$: wrapClass = isTeacher ? '' : 'ml-auto mr-1';

	// ✅ 뱃지 스타일은 기존 색감 유지
	function badgeClass(text) {
		if (isTeacher) {
			if (text === '선생님')
				return 'rounded-xl bg-red-100 h-5 pb-1 text-rose-500 font-dongle text-xl font-thin px-5 leading-5';
			return 'rounded-xl bg-blue-100 h-5 pb-1 text-blue-500 font-dongle text-xl font-thin px-5 leading-5';
		} else {
			if (text === '학생')
				return 'rounded-xl bg-teal-100 h-5 pb-4 text-teal-500 font-dongle text-xl font-thin px-5 leading-5';
			return 'rounded-xl bg-purple-100 h-5 pb-4 text-purple-500 font-dongle text-xl font-thin px-5 leading-5';
		}
	}
</script>
...
<div
	id="bubble"
	in:scale={{ delay: 0, duration: 650, easing: quintOut }}
	out:scale={{ delay: 0, duration: 0 }}
	class={`mt-2 mb-4 flex flex-row bg-chat rounded-r-3xl rounded-b-3xl pt-2 pb-4 pl-3 pr-4 space-x-3 font-dodum
    w-fit max-w-[92%] sm:max-w-[85%] md:max-w-[70%] lg:max-w-[28rem] min-w-0 ${wrapClass}`}
>
	<!-- ✅ 아바타: 선생님/학생 SVG를 여기서 if로 분기 -->
	<div class="w-12 flex pt-3 justify-end">
		...
		{:else}
			...
			>
				<rect width="47.4747" height="50" fill="url(#pattern0)" fill-opacity="0.6" />
				<defs>
					<pattern id="pattern0" patternContentUnits="objectBoundingBox" width="1" height="1">
						<use xlink:href="#image0_163_1392" transform="scale(0.0106383 0.010101)" />
					</pattern>
					<image
						id="image0_163_1392"
						width="94"
	...
	<div class="flex flex-col min-w-0">
		...
		<div class="flex flex-row space-x-2">
			{#each badges as b}
				<div class={badgeClass(b)}>{b}</div>
			{/each}
		</div>

		<!-- ✅ 이름/시간: actors에서 받은 name 출력 -->
		<div class="flex flex-row">
			<div class="text-lg">{name}</div>
			<div class="text-xs pt-2 pl-2">{current_date}</div>
		</div>

		<div class="text-sm whitespace-pre-wrap break-words min-w-0">
			<slot />
		</div>
	</div>
</div>
